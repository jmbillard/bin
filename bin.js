...
---------------------------- info ----------------------------
  ...
  notes:   a multi purpose tool to for development
           ...
           4. icon object converter (light / dark icon theme)
...
function bin_ui() {
  ...
  // botões e spacers
  var stcTxt = wBin.add('statictext', [0, 0, 600, 15], '');
  ...
  wBin.onShow = function () {
    ...
    edtText.size.width = wBin.size.width - 20;
    edtText.size.height = wBin.size.height - 120;
    progressBarBin.size.width = wBin.size.width - 20;
    btnGrp.layout.layout(true);
    radGrp.children[0].value = true;
    radGrp.children[0].active = true;
    evalBtn.enabled = false;
    exportBtn.enabled = false;
    wBin.layout.layout(true);
    wBin.layout.resize();
  };

  wBin.onResizing = wBin.onResize = function () {
